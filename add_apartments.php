    ...
    if($result){
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
             if($row["apt_type_code"]==$apt_type){
                echo '<option value="'.$row["apt_type_code"].'" selected="selected">'.$row["apt_type_code"].'</option>';
             }else{
                echo '<option value="'.$row["apt_type_code"].'">'.$row["apt_type_code"].'</option>';
             }
        }
    }
    echo '</select>';   
...
//Forms info
 echo '<div class="container" style="height:60%;width:50%;margin:auto;">';
    echo '<h2>Add new Apartment </h2>' ; 
	echo'<div class="makeit">';
    echo '<p>Apartment Building :&emsp; '; 
    getBuilding($dbc,$building_id);
    echo '</p>
    <p>Apartment Type   :&emsp;&emsp;&ensp; ';
    getAptType($dbc,$apt_type);
     echo '</p>
    <p>Apartment Number :&emsp;  <input type="text" name="apt_number" size="10" maxlength="3" value="'.$apt_num.'" /><label>(Numeric)</label></p>
    <p>Bedroom Count    : &emsp;&emsp;&ensp; <input type="text" name="bedroom_count" size="10" maxlength="1" value="'.$bed_count.'" /><label>(Numeric)</label></p>
    <p>Bathroom Count   : &emsp;&emsp; <input type="text" name="bathroom_count" size="10" maxlength="1" value="'.$bath_count.'" /><label>(Numeric)</label></p>
    <p>Total rooms      : &emsp;&emsp;&emsp;&emsp;  <input type="text" name="room_count" size="10" maxlength="1" value="'.$room_count.'" /><label>(Numeric)</label></p>';
...
if (isset($_POST['submitted'])) {
    ...
    if (empty($errors)) { 
        ...
        $query = "INSERT INTO Apartments VALUES ($apt_id,$building_id,'$apt_type',$apt_num,$bath_count,$bed_count,$room_count,'VACANT')";
